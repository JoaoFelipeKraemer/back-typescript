...
export function getLosangleArea(diagonalmaior: number, diagonalmenor: number): number {
    return (diagonalmaior * diagonalmenor) / 2
  }   

export function getTrapezioArea(baseMaior: number, base: number, height: number): number {
    return ((baseMaior + base) * height) / 2
  }
